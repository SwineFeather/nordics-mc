-- Comprehensive fix for town achievements with proper permissions and type casting
-- Addresses the integer = uuid error and adds moderator permissions
...
-- Create a function to check if user can claim town achievements
...
BEGIN
  RETURN EXISTS (
    ...
    WHERE (t.id::TEXT = p_town_id OR t.id = p_town_id::UUID)
    AND t.mayor = p.minecraft_username
  );
END;
$$;

-- Create a single, properly typed claim function with permissions
...
DECLARE
  ...
  can_claim BOOLEAN;
BEGIN
  -- Check if user can claim achievements for this town
  SELECT public.can_claim_town_achievement(p_town_id) INTO can_claim;
  ...
  -- Check if tier exists (handle both UUID and TEXT tier IDs)
  ...
  SELECT EXISTS(
    ...
    WHERE town_id = p_town_id 
      AND (tier_id::TEXT = p_tier_id OR tier_id = p_tier_id::UUID)
...
FROM pg_proc 
WHERE proname IN ('claim_town_achievement', 'can_claim_town_achievement')

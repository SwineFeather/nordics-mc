-- Fix the sync_all_town_achievements function to work with the actual towns table structure

-- Drop the existing function if it exists
DROP FUNCTION IF EXISTS public.sync_all_town_achievements();

-- Create the corrected function that handles missing columns
...
DECLARE
  ...
  has_is_independent BOOLEAN := FALSE;
BEGIN
  -- Check if is_independent column exists
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'towns' 
    AND column_name = 'is_independent'
    AND table_schema = 'public'
  ) INTO has_is_independent;
    ...
    FOR achievement_record IN SELECT * FROM public.town_achievement_definitions LOOP
      ...
      CASE achievement_record.stat
        ...
        WHEN 'independent' THEN
          -- Handle missing is_independent column
          IF has_is_independent THEN
            town_stat_value := CASE WHEN town_record.is_independent THEN 1 ELSE 0 END;
          ELSE
            -- If is_independent column doesn't exist, assume independent if no nation_id

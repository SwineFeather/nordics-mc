...
type StatKey = keyof Omit<PlayerProfile['stats'], 'ranks' | 'balance'>;

interface StatLeaderboardModalProps {
  statKey: string;
  statLabel: string;
  onClose: () => void;
}
...
const formatStatValue = (value: number, statKey: string): string => {
  if (statKey.includes('play_time') || statKey === 'playtimeHours') {
    // Convert ticks to hours for display
    const hours = statKey === 'playtimeHours' ? value : Math.floor(value / 72000);
    return `${hours}h`;
  } else if (statKey.includes('_one_cm') || statKey.includes('walk') || statKey.includes('sprint')) {
...
export const StatLeaderboardModal = ({ statKey, statLabel, onClose }: StatLeaderboardModalProps) => {
  const { leaderboardData, loading } = useStatLeaderboard(statKey);
    ...
    <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Leaderboard for the {statLabel.toLowerCase()} statistic.
          </DialogDescription>
        </DialogHeader>
              ...
              <TableBody>
                {leaderboardData.map(({ uuid, username, value, rank }) => (
                  <TableRow key={`${uuid}-${rank}`}>
                    ...
                    <TableCell className="text-right font-mono">{formatStatValue(value, statKey)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>

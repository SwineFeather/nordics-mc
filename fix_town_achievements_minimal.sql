-- Minimal fix for town achievements - only uses columns we know exist

-- Drop the existing function if it exists
DROP FUNCTION IF EXISTS public.sync_all_town_achievements();

-- Create the minimal function that only uses existing columns
...
BEGIN
  ...
  FOR town_record IN SELECT * FROM public.towns LOOP
    ...
    FOR achievement_record IN SELECT * FROM public.town_achievement_definitions LOOP
      -- Calculate town-specific stats (only using columns we know exist)
      CASE achievement_record.stat
        ...
        WHEN 'independent' THEN
          -- Assume independent if no nation_id
          town_stat_value := CASE WHEN town_record.nation_id IS NULL THEN 1 ELSE 0 END;
        WHEN 'capital' THEN
          -- For now, assume no capitals (we'll add this logic later)
          town_stat_value := 0;
        WHEN 'age' THEN
